...
if ($address1 == "") {
    $sqls = "UPDATE `user` SET `address1`='$addre' WHERE `username` LIKE '$username' ";
    $resultes = $db->query($sqls);
    $istrue = true;
} else if ($address2 == "") {
    $sqls = " UPDATE `user` SET `address2`='$addre' WHERE `username` LIKE '$username' ";
    $resultes = $db->query($sqls);
    $istrue = true;
} else if ($address3 == "") {
    $sqls = " UPDATE `user` SET `address3`='$addre' WHERE `username` LIKE '$username' ";
    $resultes = $db->query($sqls);
    $istrue = true;
}
